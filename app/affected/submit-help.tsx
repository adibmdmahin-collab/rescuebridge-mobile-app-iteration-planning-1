...
export default function SubmitHelpScreen() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (error) {
        setErrors({
          submit: "Request could not be saved to the database. Please try again.",
        });
        return;
      }
      ...
      const shortId = data?.id ? data.id.slice(0, 8) : "saved";

      setConfirmation(`Request ${shortId} was saved to Supabase with Pending status.`);
      setHelpType(null);
      setLocation("");
      setDescription("");
      setPriority(null);
      ...
      setErrors({
        submit: "Something went wrong while saving the request.",
      });
    } finally {
  ...
  return (
    <Screen>
      ...
      <SectionTitle
        title="Request Help"
        subtitle="Complete the shortest possible form. This request will be saved to Supabase."
      />
      ...
      {confirmation ? (
        <Card accentColor={COLORS.success} style={styles.confirmation}>
          ...
          <AppButton
            ...
            onPress={() => router.push("/affected/my-requests" as any)}
            variant="success"
          />
        </Card>
      ) : null}
      ...
      <AppButton
        title={isSubmitting ? "Saving..." : "Submit Request"}
        onPress={submit}
        variant="danger"
